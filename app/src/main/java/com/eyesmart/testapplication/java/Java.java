...
public class Java {
    /**
     ...
     -------------------------------------------------------------------------------------------------------------------------------
     Java 内存区域（运行时数据区）:
        线程私有的：程序计数器（记录当前线程执行的位置）、虚拟机栈（栈帧，Java 方法执行的内存模型）、本地方法栈（Native 方法栈帧）
        线程共享的：堆（对象实例和数组，垃圾回收器主要区域）、方法区（虚拟机加载的类信息、常量、静态变量等）、直接内存

        堆内存分类：新生代、老生代、永生代。永生代是方法区的实现，永生代（方法区）1.8被元空间取代（直接内存）， 这样可不受JVM本身设置固定大小空间的限制
